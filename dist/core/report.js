...
// ─── Text Report ──────────────────────────────────────────────────
function textReport(result, noColor = false) {
    const c = noColor
    ...
    if (result.serverInfo.framework)
        lines.push(`${c.cyan}Framework:${r} ${result.serverInfo.framework}`);
    if (result.serverInfo.transportType)
        lines.push(`${c.cyan}Transport:${r} ${result.serverInfo.transportType}`);
    ...
    if (s.critical > 0)
        lines.push(`  ${SEVERITY_ICONS.critical} Critical: ${s.critical}`);
    if (s.high > 0)
        lines.push(`  ${SEVERITY_ICONS.high} High:     ${s.high}`);
    if (s.medium > 0)
        lines.push(`  ${SEVERITY_ICONS.medium} Medium:   ${s.medium}`);
    if (s.low > 0)
        lines.push(`  ${SEVERITY_ICONS.low} Low:      ${s.low}`);
    if (s.info > 0)
        lines.push(`  ${SEVERITY_ICONS.info} Info:     ${s.info}`);
    lines.push("");
    ...
    for (const f of result.findings) {
        ...
        if (f.cwe)
            lines.push(`  ${c.dim}CWE: ${f.cwe}${r}`);
        if (f.remediation)
            lines.push(`  ${c.green}Fix: ${f.remediation.description}${r}`);
        lines.push("");
    }
    return lines.join("\n");
}
...
// ─── Export ───────────────────────────────────────────────────────
function generateReport(result, format, noColor = false) {
    switch (format) {
